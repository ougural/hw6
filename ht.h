...
private:
    ...
    // ADD MORE DATA MEMBERS HERE, AS NECESSARY
		double alpha_; 
		unsigned int numDeleted_; 
		unsigned int numLeft_; 
       ...
       :  hash_(hash), kequal_(kequal), prober_(prober)
{
    // Initialize any other data members as necessary
		//initializing table with 11 empty locations
		mIndex_ = 0;
		alpha_ = resizeAlpha;
		numDeleted_ = 0;
		numLeft_ = 0;
...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
	// while(table_.size() != 0)
	// {
	// 	//if the last index has an item then delete 
	// 	if (table_.back() != nullptr)
	// 	{
	// 		delete table_.back();
	// 	}
	// 	table_.pop_back();
...
{
	//if there's nothing else to delete than its empty
	if(numDeleted_ == 0){
		return 1;
	}
	else{
...
{
	if (double(numDeleted_ + numLeft_) / double(table_.size()) >= alpha_)
	{
		resize();
	}
	HASH_INDEX_T h = this->probe(p.first);
	if (h == npos){
		throw std::logic_error("Could not insert");
	}
...
{
    std::vector<HashItem*> resized(CAPACITIES[++mIndex_], nullptr);
	...
	while(old.size() != 0)
	{
		if(old.back() != nullptr && old.back() -> deleted == 0)
		{
			insert(old.back() -> item);
			delete old.back();
		}
		old.pop_back();
	}
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        // return 'loc' if the given key exists at this location
				//move on, remove -> technically not deleting, not inserting to something that is already deleted)
